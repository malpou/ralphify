...
- Check off the completed step in PLAN.md
- Commit the PLAN.md update

If you hit a blocker that requires human input, write a question to QUESTIONS.md under `## Open` with format: `- **<task-slug>**: <question>` and stop.
...
- Create a draft PR: `gh pr create --draft --title "<title>" --body "<plan summary and what was done>"`
- Update TODO.md: move the task from `## Active` to `## Done`, mark it `[x]`, note the PR number
- Switch back to main: `git checkout main && git pull`

### 5. Inbox has unprocessed items?

Pick the TOP unchecked item from INBOX.md:
...
- Create the directory `tasks/<slug>/`
- Write `tasks/<slug>/PLAN.md` with:
  - `# Plan: <title>`
...
- Add the task to TODO.md under `## Active`: `- [ ] <slug> — <short description> (see tasks/<slug>/PLAN.md)`
- Mark the inbox item as `[x]` in INBOX.md
- Create and switch to a new branch: `git checkout -b pm/<slug>`
- Commit the plan files: `git add . && git commit -m "plan: <slug> — <short description>"`

If the work item is too large (would need more than 8 plan steps), split it into multiple smaller items in INBOX.md instead.
